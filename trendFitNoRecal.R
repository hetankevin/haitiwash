...
haiti1mod <- function(vacscen = 'id0', period = 'epidemic') {
  ...
  beta <- "double mybeta = beta1*seas1 + beta2*seas2 + beta3*seas3 + beta4*seas4 + beta5*seas5 + beta6*seas6 + exp(betat*((t-215)/(430-215))); \n "
  if (depts > 1) {
  ...
  model1 <- pomp::window(model1, start = 1, end = 430) ## epidemic period
  model1@t0 <- 0
...
# S_0 ??
unfixed_param_names <- c("beta1", "beta2", "beta3", "beta4", "beta5", "beta6", "betat",
...
rw_sd1 <- rw.sd(beta1 = .02, beta2 = .02, beta3 = .02,
                beta4 = .02, beta5 = .02, beta6 = .02, betat=0.02,
...
save(results, file = "trendResultsJoint.rda")
