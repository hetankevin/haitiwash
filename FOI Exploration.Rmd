...
results
epi_mod_1 <- haiti1mod()
epi_mod_1@params = results[which.max(results$loglik),] %>% unlist
...
haitiRainfall %>%
  ...
  labs(title = expression("Weekly Rainfall from 2014-2019"), x='Week', y='Rainfall (mm)')
...
epi_sims %>% group_by(week) %>% summarize(beta = quantile(beta, c(0.025, 0.5, 0.975), 
  ...
  pivot_wider(names_from=q, values_from=beta) %>% mutate(
    date_week_start = ymd(paste0(2010, "-10-23")) + weeks(week - 1),
    week_date = as.Date(date_week_start, format = "%m-%d"),
    Year=as.factor(year(week_date))) %>% group_by(week) %>% 
  filter(week_date > '2010-12-31') %>% group_by(Year) %>% 
