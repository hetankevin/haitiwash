...
```{r}
helper <- resultsRecal %>% filter(full_loglik==new_max)
params <- unlist(helper[, c(3:28)])
```
...
sigma <- mod_1@params[['sigma']]
mu <- mod_1@params[['mu']]
delta <- mod_1@params[['delta']]
gamma <- mod_1@params[['gamma']]

sims <- sims %>% na.omit() %>%
  mutate(lambda = (I + (1-kappa)*A)^mod_1@params[['nu']] * (beta/N)) %>%
  mutate(r0 = (N*sigma * lambda * mu) / (delta * (sigma+delta) * (gamma + delta)))
```
...
for(i in c(1:max(sims$week))) {
  med_r0[i] <- median(filter(sims, week==i)$r0)
}
r0_df <- data.frame(med_r0, week = c(1:max(sims$week)))
```


```{r}
(end <- r0_df  %>% ggplot(aes(x = week, y = med_r0)) + geom_point() + labs(title = "Endemic Sims Median FOI by Week"))
```
