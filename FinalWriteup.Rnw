...
This is a nontrivial task, given that existing models do not attempt to factor in the possibility of a trend and numerous confounding factors, such as a depletion of susceptibles.
Ultimately, we find evidence for a decreasing trend in the transmissibility of cholera based on the first model featured in Lee et al. \cite{Lee_haiticholera}, via hypothesis testing on parameters estimated via simulation-based inference. 
