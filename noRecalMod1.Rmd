...
Check that everything is working. High loglik implies we are evaluating lik over the entire window.
...
Note the run time took a lot longer than for when we were just looking at the epidemic window. This makes sense since there is more data to consider with the bigger window.
...
Use trendResults.rda data frame as starting point for the combined profile going into the endemic period. We will refit rho, tau, and sigma^2. 
...
system.time(foreach(i = 1:nrow(guesses), .combine=rbind) %dopar% {
 fixed_params <- unlist(results[i,c("beta1", "beta2", "beta3", "beta4", "beta5", "beta6", "pop_0","mu", "delta", "sigma", "gamma", "alpha", "theta0", "A_0", "R_0","nu", "E_0", "I_0")])
  ...
  mod_1 %>%
    ...
    mif2(
      rw.sd=rw_sd1, Nmif=1,cooling.fraction.50=0.5, Np=1) -> mf
...
print('starting fitting')
#foreach(guess=iter(guesses,"row"), .combine=rbind) %dopar% {
system.time(foreach(i = 1:37, .combine=rbind) %dopar% {
  fixed_params <- unlist(results[i,c("beta1", "beta2", "beta3", "beta4", "beta5", "beta6", "pop_0","mu", "delta", "sigma", "gamma", "alpha", "theta0", "A_0", "R_0","nu", "E_0", "I_0")])
  pars = c(unlist(guesses[i,]),fixed_params, S_0 = 1 - unlist(unname(results[i,"E_0"])) - unlist(unname(results[i,"I_0"])))
  ...
  mod_1 %>%
    mif2(
      rw.sd=rw_sd1, Nmif=Nmif,cooling.fraction.50=0.5, Np=1) -> mf
  # replicate 10 runs of pfilter so we can get se's
  print('replicating guess')
  print(i)
  replicate(
