...
betatSamps <- freeze(seed=1196696958,
                     profile_design(
                       betat=seq(-0.1,0.1,length=100),
...
foreach(start = iter(starts_if, by = 'row'), 
        ...
        .packages = c("pomp", "tidyverse", "magrittr")) %dopar% {
          ...
          epi.lik <- replicate(Nreps_eval, pfilter(epi_mod, Np = Np))
          end.lik <- replicate(Nreps_eval, pfilter(end_mod, Np = Np))
          ...
          rm(mf.mod, epi_mod, end_mod, epi.lik, end.lik, full.lik)
          gc()
          dummy
}-> resultsRecal

save(resultsRecal, file = "trendProfileRecalResults2.rda")
